...
class FundTransfer(BasePage):
    ...
    def enter_money(self,money,msg):
        ...
        success_msg=self.find_web_element(self.SUCCESS_MSG).text
        assert "You successfully submitted your transaction." in success_msg
        print(f"{success_msg} visible successfully")
        transaction_msg=self.find_web_element(self.COMPLETE_TRANSACTION).text
        print(f'{transaction_msg} visible successfully')
